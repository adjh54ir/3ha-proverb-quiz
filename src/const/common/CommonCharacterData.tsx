...
//
// ─────────────────────────────────────────────────────────────
// 문제 풀이 수별 캐릭터(등급) 시스템 — 단일 소스(Single Source of Truth)
// ─────────────────────────────────────────────────────────────
// - 앱 전역에서 사용하는 "캐릭터(마스코트)" 데이터를 이 파일에서만 정의한다.
// - 등급 취득 조건은 "누적 풀이 문제 수"(0/100/200/500/1,000/2,000문제) 기준이며,
//   내부 임계 점수 = 필요 문제 수 × 문제당 점수(SCORE_PER_QUESTION)로 산정한다.
...
export interface CharacterLevel {
	...
	/** 등급 대표 색상 */
	color: string;
	/** 이 등급을 획득하는 데 필요한 누적 풀이 문제 수(이상) */
	questions: number;
	/** 등급 마스코트 이미지 */
	mascot: any;
	/** 짧은 격려 문구 */
...
const BASE_LEVELS: CharacterLevel[] = [
	{
		...
		encouragement: '🌱 첫걸음을 내디뎠습니다!\n앞으로가 더욱 기대됩니다!',
		description: '속담 학습의 출발선에 선 단계로,\n새싹처럼 작은 배움부터 차근차근 키워가는 시기입니다.\n앞으로의 성장이 더욱 기대됩니다.',
	},
	{
		id: 2,
		label: '속담 입문자',
	...
	},
	{
		...
		encouragement: '🌳 지식이 뿌리내려 점점 자라고 있습니다!\n이제 훨씬 더 능숙해졌습니다!',
		description: '속담의 의미와 쓰임새를 제대로 이해하고,\n실전에서도 능숙하게 활용할 수 있는 단계입니다.\n기초를 넘어 한층 성숙한 실력을 갖췄습니다.',
	},
	{
		id: 4,
		label: '속담 고수',
		...
		encouragement: '⚔️ 속담의 전장에서 승리하고 있습니다!\n어떤 도전도 당당히 맞설 수 있습니다!',
		description: '속담을 무기처럼 활용하며,\n어려운 문제도 당당히 맞설 수 있는 단계입니다.\n탄탄한 자신감으로 진정한 실력을 보여줍니다.',
	},
	{
		id: 5,
		label: '속담 마스터',
...
/**
 * 캐릭터(등급) 데이터를 생성한다.
 * 임계 점수 = "필요 풀이 문제 수 × 문제당 점수".
 * (예: 100문제 → 1,000점 / 2,000문제 → 20,000점)
...
export const buildLevelData = (): ScoredCharacterLevel[] => {
	...
	const ascending = BASE_LEVELS.map((lv) => ({
		...lv,
		score: lv.questions * SCORE_PER_QUESTION,
	}));
