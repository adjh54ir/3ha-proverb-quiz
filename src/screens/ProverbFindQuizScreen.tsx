...
const ProverbFindQuizScreen = () => {
	...
	const loadQuestion = (data: MainDataType.Proverb[]) => {
		...
		const distractors = shuffled.slice(1, 4).map((item) => item.proverb);
		const allOptions = [...distractors, newQuestion.proverb].sort(() => Math.random() - 0.5);
	...
	const handleSelect = (answer: string) => {
		...
		const correct = answer === question.proverb;
		setSelected(answer);
		setIsCorrect(correct);

		setResultTitle(correct ? '정답입니다!' : '오답입니다');
		setResultMessage(correct ? '잘했어요! 🎯' : `정답: ${question.proverb}`);
			...
			<TouchableWithoutFeedback onPress={Keyboard.dismiss}>
				<View style={styles.container}>
					<View style={styles.quizBox}>
						...
						<Text style={styles.questionText}>{question?.meaning || '문제 준비중...'}</Text>
...
export default ProverbFindQuizScreen;
