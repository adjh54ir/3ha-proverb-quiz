	...
	const renderItem = ({ item }: { item: MainDataType.Proverb }) => {
		...
		return (
			<View style={styles.quizBox}>
				{/* 문제 + 정답/오답 결과 표시 */}
				<View style={styles.quizSubContainer}>
					...
					{result !== undefined && <Text style={result ? styles.correct : styles.wrong}>{result ? '⭕ 정답!' : '❌ 오답'}</Text>}
				</View>
						...
						{!isQuizCompleted ? (
							...
							<>
								...
								<TouchableOpacity onPress={() => setShowTodayReview((prev) => !prev)} style={styles.reviewToggleButton}>
									<IconComponent name={showTodayReview ? 'chevron-up' : 'chevron-down'} type="FontAwesome" size={16} color="#333" style={{ marginRight: 8 }} />
									<Text style={styles.acodianTxt}>{showTodayReview ? '오늘 문제 접기' : '오늘 문제 다시 보기'}</Text>
								</TouchableOpacity>
