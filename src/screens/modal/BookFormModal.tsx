			...
			<ScrollView horizontal showsHorizontalScrollIndicator={false} style={pickerStyles.scrollArea}>
				<View>
					{colorRows.map((row, ri) => (
						<View key={ri} style={pickerStyles.row}>
							{row.map((color) => (
								<TouchableOpacity key={color} style={[pickerStyles.colorDot, { backgroundColor: color }, selectedColor === color && pickerStyles.colorDotSelected]} onPress={() => onColorChange(color)}>
				...
				<ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
					<View style={styles.modal}>
						...
						<View style={[pickerStyles.preview, { backgroundColor: color + '20', borderColor: color + '40', width: '100%', marginTop: scaleHeight(12) }]}>
							...
							<View style={{ flex: 1 }}>
								<Text style={[pickerStyles.previewLabel, { color }]} numberOfLines={1}>
									{title.trim() || '속담집 이름'}
								</Text>
								{desc.trim() ? (
									<Text style={pickerStyles.previewDesc} numberOfLines={1}>
										{desc.trim()}
									</Text>
								) : null}
							</View>
						...
						<TouchableOpacity style={[styles.submitBtn, { backgroundColor: '#3B82F6', marginTop: scaleHeight(16) }, !title.trim() && styles.submitBtnDisabled]} onPress={handleSubmit} disabled={!title.trim()}>
