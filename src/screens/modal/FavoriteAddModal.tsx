...
const CATEGORY_ITEMS = [
	{ label: '전체', value: '전체', iconType: 'FontAwesome6', iconName: 'clipboard-list', iconColor: '#64748B' },
	{ label: '운/우연', value: '운/우연', iconType: 'FontAwesome6', iconName: 'dice', iconColor: '#81ecec' },
	{ label: '인간관계', value: '인간관계', iconType: 'FontAwesome6', iconName: 'users', iconColor: '#a29bfe' },
	{ label: '세상 이치', value: '세상 이치', iconType: 'fontawesome5', iconName: 'globe', iconColor: '#fdcb6e' },
	{ label: '근면/검소', value: '근면/검소', iconType: 'fontawesome5', iconName: 'hammer', iconColor: '#fab1a0' },
	{ label: '노력/성공', value: '노력/성공', iconType: 'fontawesome5', iconName: 'medal', iconColor: '#55efc4' },
	{ label: '경계/조심', value: '경계/조심', iconType: 'fontawesome5', iconName: 'exclamation-triangle', iconColor: '#ff7675' },
	{ label: '욕심/탐욕', value: '욕심/탐욕', iconType: 'fontawesome5', iconName: 'hand-holding-usd', iconColor: '#fd79a8' },
	{ label: '배신/불신', value: '배신/불신', iconType: 'fontawesome5', iconName: 'user-slash', iconColor: '#b2bec3' },
];
...
const FavoriteAddModal = ({ visible, existingIds, onClose, onAdd }: Props) => {
	...
	const [categoryItems, setCategoryItems] = useState(
		CATEGORY_ITEMS.map((v) => ({
			...v,
			icon: () => <IconComponent type={v.iconType} name={v.iconName} size={scaledSize(16)} color={v.iconColor} />,
	...
	const baseList = useMemo(() => CONST_MAIN_DATA.PROVERB.filter((p) => !existingIds.includes(p.id)), [existingIds, visible]);

	const filteredList = useMemo(() => {
		let list = [...baseList];
		if (keyword.trim()) {
			const lower = keyword.toLowerCase();
			list = list.filter((item) => item.proverb?.toLowerCase().includes(lower) || item.meaning?.toLowerCase().includes(lower) || item.longMeaning?.toLowerCase().includes(lower));
		}
		if (levelValue !== '전체') {
	...
	const renderItem = ({ item, index }: { item: MainDataType.Proverb; index: number }) => {
		...
		const categoryMeta = CATEGORY_ITEMS.find((c) => c.value === item.category);
			...
			<TouchableOpacity
				...
				<View style={styles.itemHeader}>
					<View style={styles.badgeRow}>
						<View style={[styles.levelBadge, { backgroundColor: LEVEL_COLOR_MAP[item.level] ?? '#b2bec3' }]}>
							<IconComponent type="FontAwesome6" name={LEVEL_ICON_MAP[item.level] ?? 'circle'} size={scaledSize(10)} color="#fff" />
							<Text style={styles.badgeText}>{LEVEL_NAME_MAP[item.level] ?? item.levelName}</Text>
						</View>
						<View style={[styles.categoryBadge, { backgroundColor: CATEGORY_COLOR_MAP[item.category] ?? '#b2bec3' }]}>
							{categoryMeta && categoryMeta.iconName !== 'clipboard-list' && (
								<IconComponent type={categoryMeta.iconType} name={categoryMeta.iconName} size={scaledSize(10)} color="#fff" />
							)}
				...
				<KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.sheet}>
					<View style={styles.modalHeader}>
						...
						<Text style={styles.headerSubText}>
							추가할 속담을 선택해주세요 <Text style={styles.headerCount}>({selectedIds.length})</Text>
						</Text>
					</View>
								...
								<View style={styles.dropdownRow}>
									...
									<View style={[styles.dropdownWrapper, { zIndex: categoryOpen ? 3000 : 1000 }]}>
										<DropDownPicker
											...
											closeIconStyle={{ width: scaleWidth(24), height: scaleWidth(24) }}
...
const styles = StyleSheet.create({
	...
	proverbText: { fontSize: scaledSize(16), fontWeight: '700', color: '#334155', marginBottom: scaleHeight(4) },
