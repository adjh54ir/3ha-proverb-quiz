...
const FavoriteAddModal = ({ visible, existingIds, onClose, onAdd }: Props) => {
	...
	useEffect(() => {
		...
		setCategoryItems([
			...
			...cats.map((c) => ({
				...
				labelStyle: { marginLeft: SPACING_W.xsPlus, fontSize: FONT_SIZES.md },
			})),
		]);
	}, []);
	...
	const baseList = useMemo(() => ProverbServices.selectProverbList().filter((p) => !existingIds.includes(p.id)), [existingIds, visible]);
								...
								<View style={styles.dropdownRow}>
									<View style={[styles.dropdownWrapper, { zIndex: levelOpen ? 3000 : 1000 }]}>
										<DropDownPicker
											...
											listItemLabelStyle={{ marginLeft: SPACING_W.xsPlus, fontSize: FONT_SIZES.smPlus }}
...
const styles = themedStyles(() => StyleSheet.create({
	...
	sheet: { height: '92%', maxHeight: '100%', backgroundColor: COLORS.background, borderTopLeftRadius: RADIUS.xl, borderTopRightRadius: RADIUS.xl, overflow: 'hidden' },
